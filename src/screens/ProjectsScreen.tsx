  ...
  return [...list]
   ...
   async function exportCSV() {
    ...
    const rows = withSequentialSn(filtered).map((p) => {
      ...
      p.project_name,
      // Project info
      p.po_number, p.po_value_sar, p.region, p.sector, p.plan_no, p.city, p.project_type, p.latitude, p.longitude, p.project_manager,
      ...
      getVal(p, "stage4", "connectScanStatus"),
...
placeholder="Search by name, PO No., Plan No., Site ID, Project Manager, Contractor..."
     ...
     <div className="overflow-auto rounded-xl border border-ink-700 bg-ink-800" style={{ maxHeight: "calc(100vh - 0px)" }}>
