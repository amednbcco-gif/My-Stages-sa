...
function MilestoneCard({
  ...
  return (
    <div className={`rounded-2xl border bg-ink-800 overflow-hidden transition-all ${done ? "border-emerald-500/30" : "border-ink-700"}`}>
      ...
      <div className="px-5 py-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3">          {milestone.fields.map((field) => {            const value = stageData[field.key] ?? "";            return (              <div key={field.key} className="flex flex-col gap-1">                <label className="text-[11px] font-semibold text-white">{field.label}</label>                <FieldInput field={field} value={value} stage={milestone.stage} onFieldChange={onFieldChange} disabled={!canEdit} />              </div>            );          })}        </div>      )}
