...
import {
  ...
  MessageSquare, Send, X, Check, LayoutGrid, List, Plus, ExternalLink, Calculator, GanttChartSquare,
} from "lucide-react";
...
function FieldInput({ field, value, stage, onFieldChange, disabled }: FieldInputProps) {
           const isStatus = ["status","patsub","pat-status","crq-ho","close-permit","permit","clearance","done","execution","repat-status","connect-scan","trace360","sharepoint"].includes(field.type);
...
function StageCard({ stage, project, attachments, uploading, onFieldChange, onUpload: _onUpload, onDeleteAttachment, onDownloadAttachment, onSaveStage, canEdit, permits }: StageCardProps) {
  ...
  <div className="px-5 pb-3 space-y-0 divide-y divide-ink-700/40">
        ...
        <div className="border-t border-ink-700/50 px-5 py-3">
          <p className="mb-2 text-xs font-semibold text-gray-400">Permits</p>
          {permits.length === 0 ? (
            ...
            <div className="overflow-x-auto -mx-1 px-1">
                ...
                <thead>
                  <tr className="border-b border-ink-700 text-gray-500">
                    ...
                    <th className="py-1.5 px-2 text-left font-semibold">Issued Date</th>
                    <th className="py-1.5 px-2 text-left font-semibold">Start Date</th>
      ...
      <div className="overflow-x-auto -mx-2 px-2">
        <table className="w-full min-w-[820px] border-collapse text-xs">
          <thead>
              ...
              <th className="py-2 px-2 text-left font-semibold w-20">CW (m)</th>
              <th className="py-2 px-2 text-left font-semibold">Permit Status</th>
              <th className="py-2 px-2 text-left font-semibold min-w-[180px]">Note</th>
 ...
 <tr><td colSpan={9} className="py-4 text-center text-gray-600">No permits yet.{canEdit ? " Click + to add one." : ""}</td></tr>
              ...
              permits.map((p) => (
                <tr key={p.id} className="border-b border-ink-700/40 hover:bg-ink-800/40 transition-colors">
                  ...
                  <td className="py-1.5 px-2">
                    {canEdit ? <input type="number" className={inputCls + " w-20"} value={p.cw_meters} onChange={(e) => onPermitUpdate(p.id, { cw_meters: parseFloat(e.target.value) || 0 })} /> : <span className={roCls + " w-20 block"}>{p.cw_meters}</span>}
                  </td>
                     <td className="py-1.5 px-2">
                    {canEdit ? (
                      <select
